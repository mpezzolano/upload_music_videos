...
class lc_taxonomy extends WP_Widget {
    ...
    function lc_taxonomy() {
        parent::__construct('lc_taxonomy', $name = 'Video Category');
    }
    ...
    function widget($args, $instance) {
        ...
        if ($dropdown) {
            ...
            $args = array(
                ...
                'taxonomy' => $tax,
                'hide_if_empty' => true,
                'walker' => new videocraft_category_widget()
            );
    ...
    function form($instance) {
        ...
        ?>
        <p>
            <label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title:', 'videocraft'); ?></label>
            <input id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" class="widefat" />
        </p>
        <p>
            <label for="<?php echo $this->get_field_id('taxonomy'); ?>"><?php _e('Select Taxonomy:', 'videocraft'); ?></label>
            <select name="<?php echo $this->get_field_name('taxonomy'); ?>" id="<?php echo $this->get_field_id('taxonomy'); ?>" class="widefat" style="height: auto;" size="4">
                ...
                foreach ($taxonomies as $taxonomy) {
                    ?>
                    <option value="<?php echo $taxonomy; ?>" <?php
        ...
        <div class="lctw-all-options">
            ...
            <label for="<?php echo $this->get_field_id('count'); ?>"><?php _e('Show post counts', 'videocraft'); ?></label><br />
            <input type="checkbox" class="checkbox" id="<?php echo $this->get_field_id('hierarchical'); ?>" name="<?php echo $this->get_field_name('hierarchical'); ?>"<?php checked($hierarchical); ?> />
            <label for="<?php echo $this->get_field_id('hierarchical'); ?>"><?php _e('Show hierarchy', 'videocraft'); ?></label></p>				
        <p>
            <label for="<?php echo $this->get_field_id('orderby'); ?>"><?php _e('Order By:', 'videocraft'); ?></label>
        ...
        <p>
            <label><input type="radio" name="<?php echo $this->get_field_name('ascdsc'); ?>" value="asc" <?php
                if ($ascdsc == 'asc') {
                    echo 'checked';
                }
                ?>/> Ascending</label><br/>
            <label><input type="radio" name="<?php echo $this->get_field_name('ascdsc'); ?>" value="desc" <?php
                if ($ascdsc == 'desc') {
                    echo 'checked';
                }
                ?>/> Descending</label>
        ...
        <label for="<?php echo $this->get_field_id('dropdown'); ?>"><?php _e('Display as Dropdown', 'videocraft'); ?></label></p>
        </div>
        <?php
    }
...
class videocraft_category_widget extends Walker {
    ...
    function start_el(&$output, $term, $depth = 0, $args = array(), $current_object_id = 0) {
        ...
        $output.= "\t" . '<option' . ' class="' . esc_attr($class_name) . '" value="' . $url . '">' . esc_html($text) . '</option>' . "\n";
    }
