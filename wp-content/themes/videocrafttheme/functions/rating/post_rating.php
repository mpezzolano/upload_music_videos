...
$rating_path = get_template_directory_uri() . '/functions/rating/';
...
/**
 * Function Name: videocraft_save_comment_rating
 ...
 */
function videocraft_save_comment_rating($comment_id = 0) {
...
add_action('wp_insert_comment', 'videocraft_save_comment_rating');

/**
 * Function Name: videocraft_del_comment_rating
 ...
 * @param type $comment_id 
 */
function videocraft_del_comment_rating($comment_id = 0) {
...
add_action('wp_delete_comment', 'videocraft_del_comment_rating');

/**
 * Function Name: videocraft_draw_rating_star
 ...
 * @return string 
 */
function videocraft_display_rating_star($avg_rating) {
...
/**
 * Function Name: videocraft_get_rating
 ...
 * @global string $rating_table_name 
 */
function videocraft_get_rating() {
    global $post, $rating_path, $rating_star_on, $rating_star_off, $rating_table_name;
    videocraft_rating_js();
    for ($i = 1; $i <= POSTRATINGS_MAX; $i++) {
        if ($i == 1) {
            $rating_text = $i . ' rating';
        } else {
            $rating_text = $i . __(' rating', 'videocraft');
        }
...
//		echo '<img src="'.$rating_image_off.'" class="rating_img" onmouseover="current_rating_star_on(\''.$post->ID.'\',\''.$i.'\',\''.$rating_text.'\');" onmousedown="current_rating_star_off(\''.$post->ID.'\',\''.$i.'\');" id="rating_'.$post->ID.'_'.$i.'"  alt="" />';							
    ...
    echo '<script type="text/javascript">current_rating_star_on(\'' . $post->ID . '\',0,\'0 ' . __('rating', 'videocraft') . '\');</script>';
}

/**
 * Function Name: videocraft_get_post_average_rating
 ...
 * @return type 
 */
function videocraft_get_post_average_rating($pid) {
...
/**
 * Function Name: videocraft_get_post_rating_star
 * Description: Display rating in post
 * @param type $pid
 * @return type 
 */
function videocraft_get_post_rating_star($pid = '') {
    $rtn_str = '';
    $avg_rating = videocraft_get_post_average_rating($pid);
    $rtn_str = videocraft_display_rating_star($avg_rating);
    return $rtn_str;
}

/**
 * Function Name: videocraft_is_user_can_add_comment
 ...
 * @return type 
 */
function videocraft_is_user_can_add_comment($pid) {
