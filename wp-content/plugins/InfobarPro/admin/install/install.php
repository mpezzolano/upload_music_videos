...
function nb_install() {
    ...
    if (get_option("pd_nb_db_version") != NB_DB_VERSION) {
		...
		placeholder_name tinytext NOT NULL,
		placeholder_email tinytext NOT NULL,
...
function nb_install_example_notifications() {
    ...
    $wpdb->insert($table_name_notifications, array('name' => __('IMPORTANT', 'infobar'), 'icon' => NB_DIR . '../../images/important.png', 'bgcolor' => 'rgb(17, 130, 184)', 'fontcolor' => 'white'));
    $wpdb->insert($table_name_notifications, array('name' => __('ANNOUNCEMENT', 'infobar'), 'icon' => NB_DIR . '../../images/announcement.png', 'bgcolor' => '#B61522', 'fontcolor' => 'white'));
}
...
function nb_subscriber_list(){
 global $wpdb;
 $table_name_subscriberlist = $wpdb->prefix . "nb_subscriber_list";
        $sqlsubscriberlist = "CREATE TABLE IF NOT EXISTS " . $table_name_subscriberlist . " (
		id mediumint(9) NOT NULL AUTO_INCREMENT,
		name tinytext NOT NULL,
		email text NOT NULL,
		PRIMARY KEY (`id`)
       ...
       // require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
        $wpdb->query($sqlsubscriberlist);
}
add_action('plugins_loaded', 'nb_subscriber_list');
?>