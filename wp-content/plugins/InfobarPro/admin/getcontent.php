...
function nb_get_content() {
    ...
    $sql = "SELECT name,epochtimebegin,epochtimeend,timebegin,timeend,content,bgcolor,fontcolor,nottype,selected_option,placeholder_name,placeholder_email,button_text FROM " . $table_name . " WHERE ('" . $phptime . "'  >= epochtimebegin AND '" . $phptime . "' <= epochtimeend AND status = 1) OR (epochtimebegin IS NULL AND status = 1) ORDER BY epochtimeend";
    if (get_option('pd_nb_debug', 'false') == 'true') {
    ...
    if (count($myrows) != 0) {
        ...
        foreach ($myrows as $myrows) {
            ...
            if (empty($tmpnotsize)) {
                $annfontsize .= '"' . get_option('pd_nb_bartextsize', '18') . 'pt",';
            } else {
        ...
        $content .= '"positioning" : "' . get_option('pd_nb_bartype', 'inline') . '",';
        ...
        $content .= '"height" : ' . get_option('pd_nb_barheight', '45') . ',';
        $content .= '"collapsedButtonHeight" : ' . get_option('pd_nb_buttonheight', '45') . ',';
        $content .= '"messagesFadeDelay" : 1500,';
        $content .= '"easing" : "' . get_option('pd_nb_easingtype', 'swing') . '",';
        $content .= '"fontSize" : "' . get_option('pd_nb_fontsize', '11') . 'pt",';
        ...
        if ($myrows->selected_option == '2') {
            global $wpdb;
            update_option('infobar_obtain_status', 'obtained');
            $info_name = $myrows->placeholder_name;
            $info_email = $myrows->placeholder_email;
            $info_button = $myrows->button_text;
            update_option('infobar_obtain_name', $info_name);
            update_option('infobar_obtain_email', $info_email);
            update_option('infobar_obtain_button', $info_button);
        } else {
            global $wpdb;
            update_option('infobar_obtain_status', 'message');
        }
    } else {
        if (get_option('pd_nb_debug', 'false') == 'true') {
